...
class SpoonService {
  async getTestApiCall() {
    ...
    let requestStr =
      "recipes/complexSearch?cuisine=italian&intolerances=gluten";
  ...
  async getIngredientsByRecipeID(recipeID) {
    ...
    try {
      ...
      ingredients = ingredients
        .map((ingredient) => nlpService.standardize(ingredient.name))
        .flat(); // flatten 2D array to 1D array
      return ingredients;
    } catch {
  ...
  async generateMealPlan(targetCalories, timeFrame, diet, excludeIngredients) {
    ...
    let requestStr =
      this.getBaseUrl() + "mealplanner/generate?" + this.getApiKeyStr();
    ...
    const res = await axios
      .get(requestStr)
      .catch((err) => console.error("Failed to generate meal plan.", err));
    if (!res) {
      return [];
    }
  ...
  async generateMealPlanSet(
    ...
    for (let i = 0; i < numPlans; i++) {
      const mealPromise = this.generateMealPlan(
        targetCalories,
        timeFrame,
        diet,
        excludeIngredients
